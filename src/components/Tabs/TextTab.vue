<template>
  <div class="space-y-4">
    ...
    <div v-else v-for="(text, index) in texts" :key="index"
      ...
      <div class="flex justify-between items-center px-2" @click="selectElement(text)">
      ...
      <div class="text-content" :class="{ 'expanded': text.isExpanded }">      
        <div class="flex justify-between mt-4">
          ...
          <div class="w-2/3">
            <input v-model="text.text" type="text" :placeholder="'Text ' + (index + 1)"
            ...
            <div class="mt-2">
              <FontSelectionVue :currentFont="text.fontFamily" @font-selected="(e) => setFontFamily(index, e)" />
            </div>

            <div class="grid gap-2 grid-cols-2 mt-2 w-full box-border">
              <div class="px-2 flex items-center bg-darker rounded-lg">
                <img class="w-4 h-4 mr-1" src='/images/assets/font-size.svg' />
              ...
              <div class="flex items-center justify-between bg-darker rounded-lg">
                <font-awesome-icon :icon="['fas', 'bold']" class="rounded-lg py-1 px-2 cursor-pointer" :class="{ 'bg-white': text.fontWeight === 'bold' }" @click="setFontWeight(index)" />
                <font-awesome-icon :icon="['fas', 'italic']" class="rounded-lg py-1 px-2 cursor-pointer" :class="{ 'bg-white': text.fontStyle === 'italic' }" @click="setFontItalic(index)" />
                <font-awesome-icon :icon="['fas', 'underline']" class="rounded-lg py-1 px-2 cursor-pointer" :class="{ 'bg-white': text.underline }" @click="setFontUnderline(index)" />
              </div>
            </div>
            <div class="grid gap-2 grid-cols-2 mt-2 w-full box-border">
              <div class="px-2 flex items-center bg-darker rounded-lg">
                <img class="w-4 h-4 mr-1" src='/images/assets/font-spacing-leftright.svg' />
                <input v-model="text.charSpacing" type="number" min="0" max="100"
                  class="min-w-0 input-element box-border" />
              </div>
              <div class="px-2 flex items-center bg-darker rounded-lg">
                <img class="w-4 h-4 mr-1" src='/images/assets/font-spacing-updown.svg' />
                <input v-model="text.lineHeight" type="number" min="0" max="100"
                  class="min-w-0 input-element box-border" />
              </div>
        ...
        <div class="mt-4 flex items-center justify-between">
            ...
            <div class="w-2/3 grid grid-cols-3 gap-2 px-2 bg-darker rounded-lg py-1 justify-items-center">
              <button class="rounded-lg w-8" :class="{ 'bg-white': text.textAlign === 'left' }"  @click="setFontLayout(index, 'left')">
                <font-awesome-icon :icon="['fas', 'align-left']" />
              </button>
              <button class="rounded-lg w-8" :class="{ 'bg-white': text.textAlign === 'center' }" @click="setFontLayout(index, 'center')">
                <font-awesome-icon :icon="['fas', 'align-center']" />
              </button>
              <button class="rounded-lg w-8" :class="{ 'bg-white': text.textAlign === 'right' }" @click="setFontLayout(index, 'right')">
        ...
        <div class="mt-4 flex items-center justify-between">
            ...
            <div class="w-2/3 px-2 bg-darker rounded-lg flex">
              <input v-model="text.fill" type="color" class="p-0.5 bg-darker color-input w-6 h-6 cursor-pointer" />
              <input v-model="text.fill" type="text" class="w-2/3 input-element" />
            </div>
        </div>
        <div class="mt-4 flex justify-between">
            ...
            <div class="w-2/3">
              <div class="bg-darker rounded-lg flex px-2">
                <input v-model="text.stroke" type="color" class="p-0.5 bg-darker color-input w-6 h-6 cursor-pointer" />
                <input v-model="text.stroke" type="text" placeholder="暂无边框" class="w-2/3 input-element" />
              </div>
              <div class="grid gap-2 grid-cols-2 mt-2 w-full box-border">
                <div class="px-2 flex items-center bg-darker rounded-lg">
                  <img class="w-4 h-4 mr-1" src='/images/assets/border-width.svg' />
                  <input v-model="text.strokeWidth" type="number" min="1" max="100"
                    class="min-w-0 input-element box-border" />
                </div>
                <div class="px-2 flex items-center bg-darker rounded-lg">
                  <img class="w-4 h-4 mr-1" src='/images/assets/border-style.svg' />
                  <select v-model="text.strokeLineCap"
